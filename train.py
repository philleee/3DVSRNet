...
parser.add_argument("--init_learning_rate", type=float, default=0.0002, help='set initial learning rate. Default: 0.0001.')
parser.add_argument("--milestones", type=list, default=[5,10,15], help='Set to epoch values where you want to decrease learning rate by a factor of 0.1. Default: [100, 150]')
...
parser.add_argument("--checkpoint_epoch", type=int, default=2, help='checkpoint saving frequency. N: after every N epochs. Each checkpoint is roughly of size 151 MB.Default: 5.')
...
trainloader = torch.utils.data.DataLoader(
    mydataset.mydataset('new_img_data', transform=transform),
    batch_size=args.train_batch_size,
    shuffle=True,
)

validationloader = torch.utils.data.DataLoader(
    mydataset.mydataset('new_img_data_val', transform=transform),
...
def validate():
    ...
    with torch.no_grad():
        for validationIndex, (validationData, validationFrameIndex) in enumerate(validationloader, 0):
            ...
            loss = 100*204 * recnLoss + 102 * warpLoss + 0.005 * prcpLoss + loss_smooth
            tloss += loss.item()
            #psnr
...
for epoch in range(dict1['epoch'] + 1, args.epochs):
    ...
    for trainIndex, (trainData, trainFrameIndex) in enumerate(trainloader, 0):
        ...
        # loss = 204 * recnLoss + 102 * warpLoss + 0.005 * prcpLoss + loss_smooth
        loss = 5*408 * recnLoss + 102 * warpLoss + 0.005 * prcpLoss + loss_smooth
    ...
    if ((epoch % args.checkpoint_epoch) == args.checkpoint_epoch - 1):
        ...
        torch.save(dict1, args.checkpoint_dir + "/3DVSRNet_" + str(checkpoint_counter) + ".ckpt")
