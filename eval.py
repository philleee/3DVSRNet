...
dict1 = torch.load('../'+args.checkpoint_dir + "/3DVSRNet1.ckpt")
...
validationloader = torch.utils.data.DataLoader(
    ...
    mydataset.mydataset('../../../data/imgs/linear/train', transform=transform),
    batch_size=1,
    shuffle=True,
)
...
def validate():
    ...
    return revNormalize(frame0.cpu()[0]),revNormalize(frameT.cpu()[0]),revNormalize(frame1.cpu()[0]),revNormalize(Ft_p.cpu()[0])
count = 0
while(count<2):
  img0,imgT,img1,valImg = validate()
  
  print('print img:'+str(count))
  ...
  # plt.show()
  plt.savefig(str(count).zfill(2)+'.jpg')
  count+=1
